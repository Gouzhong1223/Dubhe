  ...
  methods: {
    async getAtlasModels(matchData) {
      ...
      if (matchData && this.form.modelUrls.length) {
        ...
        this.form.modelUrls = this.form.modelUrls.filter((url) =>
          modelUrlList.includes(url),
        )
    ...
    async getDatasetVersions(id, matchData) {
      ...
      if (
        ...
        !this.datasetVersionList.find(
          (dataset) => dataset.versionUrl === this.form.datasetUrl,
        )
      ) {
