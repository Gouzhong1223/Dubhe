...
<template>
  ...
  >
    <div class="flex flex-between">
      ...
      <div class="btn">
        <el-button
          v-if="isDistributed && podList.length"
          @click="() => doDownloadPodLog()"
          >下载节点运行日志</el-button
        >
        <el-button
          type="primary"
...
export default {
  ...
  methods: {
    ...
    async downloadTrainLog() {
      ...
      if (!this.isDistributed) {
        ...
        if (count < LOG_DOWNLOAD_LINES_THRESHOLD) {
          const data = await batchDownloadPodLog({
            podVOList: this.podList,
          }).finally(() => {
            this.logDownloading = false
          })
          ...
          this.podList.forEach((pod) => {
            jobList.push(
              this.doDownloadPodLog(pod, logLines[pod.podName], false),
            )
          })
    ...
    async doDownloadPodLog(pod, podLogLines, divisionWarning = true) {
      ...
      const displayName =
        this.item.jobName + (this.isDistributed ? `-${pod.displayName}` : '')
      if (!podLogLines) {
