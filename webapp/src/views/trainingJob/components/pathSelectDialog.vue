...
<template>
  ...
  >
    ...
    <div v-if="fetchDone && treeList.length" class="fontBold">
      {{ description }}
    </div>
  ...
  methods: {
    ...
    async onDialogOpened() {
      const loadingInstance = Loading.service({
        target: `.${this.classKey} .tree-container`,
      })
      ;[
        this.treeList,
        this.defaultExpandedKeys,
      ] = await getTreeListFromFilepath(this.path)
      setTimeout(() => {
