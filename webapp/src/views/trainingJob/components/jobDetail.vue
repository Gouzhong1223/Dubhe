          ...
          >
            <div class="dib">
              <i
                ...
                @click="
                  goEditAlgorithm(item.algorithmId, item.algorithmCodeDir)
                "
              />
      ...
      <el-col :span="24" class="dynamic">
        ...
        <div class="text">
          <span v-for="(p, index) in runParamsList" :key="p.key"
            >{{ p.key }} = {{ p.value }}
            {{ index === runParamsList.length - 1 ? '' : ', ' }}</span
          >
        </div>
      </el-col>
      <el-col :xl="12" :span="24">
  ...
  computed: {
    ...
    outputDownloadDisable() {
      return (
        ...
        ].includes(this.item.trainStatus) || !this.item.outPath
      )
    },
    isJob() {
    ...
    spec() {
      return this.specList.find(
        (spec) => spec.specsName === this.item.trainJobSpecsName,
      )
    },
    trainModel() {
