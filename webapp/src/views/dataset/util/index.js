  ...
  try {
    ...
    result = annotation.map((d) => {
      const data = {
        ...
        color:
          (labels.find((label) => label.id === d.category_id) || {}).color ||
          '#000',
      }
      if (d.segmentation) {
...
export const cacheDatasetType = (type) =>
  localStorage.setItem('datasetListType', type)
...
export const annotationBy = (valueBy) => (value, key) => {
  const filterItems = Object.values(annotationMap).filter(
    (d) => d[valueBy] === value,
  )
  // 区分类型（是否唯一）
  if (valueBy === 'code') {
  ...
  // 基于数据类型获取内容
  if (valueBy === 'dataType') {
    const dataTypeKey = Object.keys(dataAnnotationMap).find(
      (d) => Number(d) === value,
    )
    if (dataTypeKey) {
      const items = []
...
// 筛选常见的标注类型
export const rankDatasetStatusMap = pick(datasetStatusMap, [
  101,
  102,
  104,
  105,
...
export const isStatus = (row, code) =>
  datasetStatusMap[row.status]?.status === code
...
export const isPublishDataset = (row) =>
  publishStateMap[row.dataConversion] === 'PUBLISHING'
