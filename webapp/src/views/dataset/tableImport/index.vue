  ...
  setup(props, ctx) {
    ...
    const uploadSuccess = (res) => {
      ...
      const params = {
        ...
        mergeColumn: state.checkList.map((d) =>
          state.columns.findIndex((col) => col === d),
        ),
      }
      tableImport(params)
        .then(() => {
    ...
    const processStatus = computed(() =>
      state.active === 2 && !isStatus(state.datasetInfo, 'IMPORTING')
        ? 'success'
        : 'process',
    )
    ...
    const tableData = computed(() => {
      const list = state.data
        .slice(0, 10)
        .map((d) => zipObject(state.columns, d))
      const footer = zipObject(state.columns, [
        `预览数 ${Math.min(state.data.length, 10)} / 总行数 ${
          state.data.length
        }`,
      ])
      return [...list, footer]
