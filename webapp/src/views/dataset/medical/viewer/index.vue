    ...
    <div id="dwv" v-hotkey="keymap" class="dwv-wrapper viewer rel">
      ...
      <div class="layer-content flex flex-beween">
        <div class="rel flex flex-beween" style="flex: 1;">
          <div style="width: 100%;">
            <div
              class="layerContainer mx-auto rel"
          ...
          <div v-if="state.loading" class="flex flex-center dwv-placeholder">
            加载中...
          </div>
        </div>
        <div
          v-if="state.showLesionInfo"
          class="settingsInfo"
          style="width: 22%;"
        >
          <LesionInfo
...
export default {
  ...
  setup(props, ctx) {
    ...
    const toggleToolEvent = (tool, bool) => {
      ...
      // 针对 drawlayer 要特殊处理
      const enable =
        tool.command === 'Draw' || tool.type !== 'tool' ? bool : !bool
      ...
      switch (datasetStatusMap[caseInfo.status]?.status) {
        case 'AUTO_ANNOTATED': {
          ...
          const {
            ...
            JSON.parse(autoResult.annotations),
            state.precision,
          )
          Object.assign(state, {
            sliceDrawingMap,
    ...
    const deleteDrawShape = (id) => {
      ...
      for (const group of groups) {
        ...
        if (groupToDelete.length === 1) {
          dwvApp
            .getDrawController()
            .deleteDrawGroup(groupToDelete[0], noop, noop)
        }
      }
    }
