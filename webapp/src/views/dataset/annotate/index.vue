...
export default {
  ...
  setup(props, ctx) {
    ...
    const getColorLabel = (labelId) => {
      return (
        (state.labels.find((label) => label.id === labelId) || {}).color ||
        '#000'
      )
    }
    ...
    const getLabelName = (labelId) => {
      return (
        (state.labels.find((label) => label.id === labelId) || {}).name || ''
      )
    }
    ...
    const handleSelectLabel = ({ selectedLabel, curAnnotation }) => {
      ...
      const curAnnotationIndex = state[annotationType].findIndex(
        (d) => d.id === curAnnotation.id,
      )
    ...
    const queryFile = async (id) => {
      const file =
        (await request(
          `/${API_MODULE_NAME.DATA}/datasets/files/${params.datasetId}/${id}/info`,
        )) || {}
      return file
    }
    ...
    const queryFirstImg = async () => {
      const file =
        (await request(
          `/${API_MODULE_NAME.DATA}/datasets/${params.datasetId}/files/first`,
        )) || {}
      return file
    }
    ...
    const updateImageInfo = async (fileId, labels) => {
      ...
      // annotation 可能为 null
      const annotations = file.annotation
        ? parseAnnotation(file.annotation, labels)
        : []
    ...
    onMounted(async () => {
      ...
      const activeComponent = annotationByCode(
        datasetInfo.annotateType,
        'component',
      )
      // 校验数据集标注状态
      ...
      const firstEnhanceList = await getEnhanceFileList(
        params.datasetId,
        firstFile.id,
      )
    ...
    watch(
      ...
      async () => {
        ...
        if (imgId) {
          ...
          const { annotations, file } = await updateImageInfo(
            imgId,
            state.labels,
          )
          // 跳转详情
    ...
    const currentImg = computed(() =>
      getActiveImg(state.files, state.currentImgId),
    )
