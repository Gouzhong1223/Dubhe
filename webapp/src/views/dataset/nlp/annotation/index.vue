...
export default {
  ...
  setup(props, ctx) {
    ...
    const isLast = computed(
      () =>
        !(state.pageInfo.current < state.pageInfo.total) &&
    ...
    const getStatusMap = (tab) => {
      ...
      // 默认为 unfinished
      const fileStatus =
        fileStatusKey[tab === 'finished' ? 'finished' : 'unfinished']
    ...
    const saveAnnotationUtil = (annotation) => {
      try {
        const annotationStr = isNil(annotation)
          ? null
          : JSON.stringify(annotation)
        Object.assign(state, { saving: true })
        return saveApi(params.datasetId, state.fileId, {
    ...
    const saveAction = (annotation) => {
      return saveAnnotationUtil(annotation).then(() =>
    ...
    onMounted(async () => {
      ...
      // 校验数据类型是否为文本
      if (
        ![dataTypeMap.TEXT, dataTypeMap.TABLE].includes(datasetInfo.dataTypeMap)
      ) {
