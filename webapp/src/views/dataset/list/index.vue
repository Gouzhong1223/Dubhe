                ...
                <el-tooltip effect="dark" placement="top">
                  <div slot="content">
                    当前数据集为外部导入数据集<br />点击复制数据集 ID<br /><a
                      ...
                      target="_blank"
                      :href="
                        `${VUE_APP_DOCS_URL}module/dataset/util#2-导入数据集`
                      "
                      >使用文档</a
                    >
                  </div>
...
export default {
      ...
      if (
        ...
        Promise.all(
          pollList.map((row) => {
            ...
            return this.poll(
              row,
              row.pollType || datasetStatusMap[row.status]?.status,
            )
          }),
        )
      }
  ...
  methods: {
    ...
    updateRow(id, { progress, info }) {
      ...
      if (sIndex > -1) {
        const nextRow = omit({ ...this.crud.data[sIndex], ...info, progress }, [
          'pollType',
        ])
    ...
    toggleImportDataset(visible) {
      this.importDatasetVisible = isNil(visible)
        ? !this.importDatasetVisible
        : visible
    },
    ...
    getAllFinished(progress = {}) {
      const {
        ...
        autoFinished,
        finishAutoTrack,
        annotationNotDistinguishFile,
      } = progress
      return (
        finished + autoFinished + finishAutoTrack + annotationNotDistinguishFile
      )
    },
    getProgress(row) {
      const { progress } = row
      ...
      if (['SAMPLING', 'ENHANCING', 'TRACKING'].includes(type)) {
        ...
        return this.queryDatasetDetail(row.id).then((res) => {
          if (isStatus(row, 'TRACKING')) {
            ...
            row.pollIng = true
            row.sample_progress =
              (row.sample_progress || 10) + row.sample_increment
