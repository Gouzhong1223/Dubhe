...
export default {
  ...
  methods: {
    ...
    canDelete(row) {
      return [
        RECYCLE_STATUS_ENUM.DELETION,
        RECYCLE_STATUS_ENUM.DELETED_FAIL,
      ].includes(row.recycleStatus)
    },
    ...
    async onUpdateStatus(id, option = {}) {
      ...
      if (
        this.needPoll(recycleInfo.recycleStatus) &&
        pollId &&
        this.pollMap[id] === pollId
      ) {
        setTimeout(() => {
    ...
    needPoll(status) {
      return [
        RECYCLE_STATUS_ENUM.DELETING,
        RECYCLE_STATUS_ENUM.RESTORING,
      ].includes(status)
    },

    // hook
    [CRUD.HOOK.afterRefresh]() {
      const vaildList = this.crud.data.filter((info) =>
        this.needPoll(info.recycleStatus),
      )
