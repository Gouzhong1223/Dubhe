...
export default {
  ...
  computed: {
    algorithmTypeList() {
      const types = new Set(
        this.optimizeAlgorithmList.map((algorithm) => algorithm.type),
      )
    ...
    optimizeParams() {
      return {
        ...
        type: [null, ''].includes(this.form.algorithmType)
          ? undefined
  ...
  methods: {
    ...
    async getOptimizeAlgorithms(matchData) {
      this.optimizeAlgorithmList = await getOptimizeAlgorithms(
        this.optimizeParams,
      )
