  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" width="250px" fixed="right">
        <template slot-scope="scope">
          <!--所有状态都有 -->
          <el-button type="text" @click.stop="doEdit(scope.row)"
            >编辑</el-button
          >
          <el-button type="text" @click.stop="doSubmit(scope.row.id)"
            >提交</el-button
          >
          <el-button type="text" @click.stop="doFork(scope.row)"
            >Fork</el-button
          >
          <el-button type="text" @click.stop="doDelete(scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    pageEnter(keepPageInfos) {
      if (keepPageInfos) {
        ...
        this.defaultSort = {
          prop: sort,
          order: Constant.tableSortMap2Element[order],
        }
        // 修改 crud 的排序信息
        this.crud.sort = sort
    ...
    doDelete(id) {
      this.$confirm('此操作将删除该任务, 是否继续?', '请确认').then(
        async () => {
          await deleteTask({ id })
          this.$message({
            message: '删除成功',
