    ...
    >
      <ServingModelConfig
        ...
        @model-version-change="
          (version) => onModelVersionChange(config.id, version)
        "
...
export default {
  ...
  methods: {
    ...
    onTypeChange(type) {
      // GRPC 模式不支持灰度发布
      if (
        type === ONLINE_SERVING_TYPE.GRPC &&
        this.modelConfigList.length > 1
      ) {
    ...
    onModelConfigDelete(configId) {
      const index = this.modelConfigList.findIndex(
    ...
    validateNodeCount() {
      ...
      this.nodeCountValid =
        nodeCountSum <= servingConfig.onlineServingNodeSumMax
    },
