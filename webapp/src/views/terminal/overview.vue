    ...
    <div class="base-info-wrapper flex flex-around my-20">
      <span class="base-info">
        ...
        <span class="base-info-value">{{
          dataStatistics.connectionCount
        }}</span>
      </span>
      <span class="base-info">
    ...
    <BaseTable :columns="overviewTableColumns" :data="terminalList">
      <template #name="scope">
        <el-link
          class="name-col"
          type="primary"
          @click="goConnection(scope.row.name)"
  ...
  setup(props, { root }) {
    ...
    const dataStatistics = computed(() => {
      ...
      return {
        ...
        memCount: getGiFromMi(
          runningList.reduce((count, t) => count + t.memNum, 0),
        ),
        memLimit: userConfig.memoryLimit,
        gpuCount: runningList.reduce((count, t) => count + t.gpuNum, 0),
        gpuLimit: userConfig.gpuLimit,
        diskCount: getGiFromMi(
          runningList.reduce((count, t) => count + t.diskMemNum, 0),
        ),
      }
    })
    ...
    const stopFn = () =>
      terminalList.value.filter((terminal) =>
        [TERMINAL_STATUS_ENUM.RUNNING, TERMINAL_STATUS_ENUM.SAVING].includes(
          terminal.status,
        ),
      ).length > 0
