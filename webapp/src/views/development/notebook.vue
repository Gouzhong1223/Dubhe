    ...
    >
      ...
      <el-table-column prop="status" label="状态" width="150">
        ...
        <template slot-scope="scope">
          ...
          <el-tag
            v-else
            :type="notebookTagMap[NOTEBOOK_STATUS_ENUM.STARTING]"
            effect="plain"
      ...
      <el-table-column label="操作" width="300" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="
              (isRunning(scope.row) ||
                scope.row.status === NOTEBOOK_STATUS_ENUM.STOPPED) &&
...
export default {
  ...
  methods: {
    ...
    // stop
    doStop(row) {
      this.$confirm('此操作将停止该环境, 是否继续?', '请确认').then(
        async () => {
    ...
    doDelete(row) {
      this.$confirm('此操作将删除该环境, 是否继续?', '请确认').then(
        async () => {
    ...
    showMessage(status) {
      return [
        NOTEBOOK_STATUS_ENUM.STARTING,
        NOTEBOOK_STATUS_ENUM.ERROR,
      ].includes(status)
    },
    batchStopNotebook() {
      this.$confirm('此操作将停止所有运行中的 Notebook', '请确认').then(
        async () => {
