...
export default {
  ...
  filters: {
    formatModel(item) {
      ...
      const gpuStr =
        item.gpuNum && item.spec ? ` GPU: ${item.gpuNum}*${item.spec}` : ''
  ...
  methods: {
    ...
    async getDataset(keepValue = false) {
      ...
      if (!keepValue || this.form.dataSourceId) {
        ...
        const dataset = this.datasetIdList.find(
          (dataset) => dataset.id === this.form.dataSourceId,
        )
        if (!dataset) {
