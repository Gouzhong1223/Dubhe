  ...
  <div class="app-container">
    ...
    >
      <el-tab-pane
        id="tab_0"
        label="我的镜像"
        :name="IMAGE_RESOURCE_ENUM.CUSTOM"
      />
      <el-tab-pane
        id="tab_1"
        label="预置镜像"
        :name="IMAGE_RESOURCE_ENUM.PRESET"
      />
      <el-tab-pane
        id="tab_2"
        label="Notebook 镜像"
        :name="IMAGE_RESOURCE_ENUM.NOTEBOOK"
      />
      <el-tab-pane
        id="tab_4"
        label="终端镜像"
        :name="IMAGE_RESOURCE_ENUM.TERMINAL"
      />
    </el-tabs>
    <!--表格渲染-->
    <el-table
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <i
            ...
            :style="{
              color: resourceObj(scope.row.imageResource).color,
              fontSize: '20px',
            }"
          ></i>
        </template>
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            v-if="
              hasPermission('training:image:delete') && (!isPreset || isAdmin)
            "
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        >
          ...
          >
            <el-radio :label="IMAGE_PROJECT_TYPE.TRAIN" border class="mr-0"
              >训练镜像</el-radio
            >
            <el-radio :label="IMAGE_PROJECT_TYPE.NOTEBOOK" border
              >Notebook 镜像</el-radio
            >
          </el-radio-group>
        </el-form-item>
        <el-form-item
          v-if="
            isEdit && isAdmin && form.projectType === IMAGE_PROJECT_TYPE.TRAIN
          "
          label="镜像来源"
          prop="imageResource"
        >
          <el-radio-group v-model="form.imageResource">
            <el-radio
              :label="Number(IMAGE_RESOURCE_ENUM.CUSTOM)"
              border
              class="mr-0"
              >我的镜像</el-radio
            >
            <el-radio :label="Number(IMAGE_RESOURCE_ENUM.PRESET)" border
              >预置镜像</el-radio
            >
          </el-radio-group>
        </el-form-item>
...
export default {
  ...
  cruds() {
    return CRUD({
      ...
      optShow: {
        add:
          imageConfig.allowUploadImage &&
          hasPermission('training:image:upload'),
        del: false,
      },
  ...
  methods: {
    ...
    async getImageNameList() {
      this.harborProjectList = await getImageNameList({
        projectTypes: [this.form.projectType],
      })
    },
    onImageTypeChange() {
