...
export function unixTimestamp2Normal(unixTimestamp) {
  ...
  const commonTime = unixTimestampLocal.toLocaleString('en-GB', {
    hour12: false,
  })
...
export function cpuPercentage(used, usedUnit, total, totalUnit) {
  ...
  return (
    Math.round(
      (_transition(used, usedUnit) / _transition(total, totalUnit)) * 10000,
    ) / 100
  )
}
...
export function memFormatter(
  num,
  fromUnit = 'Mi',
  toUnit = 'Gi',
  fractionDigits = 1,
) {
  return `${memNormalize(num, fromUnit, toUnit).toFixed(
    fractionDigits,
  )}${toUnit}`
}
