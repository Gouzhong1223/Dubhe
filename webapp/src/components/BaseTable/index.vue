      ...
      >
        ...
        <template v-if="column.type !== 'selection'" #default="scope">
          ...
          <div v-else-if="column.type === 'operation'">
            <template v-for="operation in column.operations">
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  >
                    ...
                    >
                      {{ moreOp.label }}
                      <IconFont
                        v-if="moreOp.iconAfter"
                        :type="moreOp.iconAfter"
                      />
                    </el-button>
                  </el-dropdown-item>
                </el-dropdown-menu>
              ...
              >
                {{ operation.label }}
                <IconFont
                  v-if="operation.iconAfter"
                  :type="operation.iconAfter"
                />
              </el-button>
            </template>
          </div>
          ...
          <el-tag
            ...
            v-bind="getTagAttrs(column, scope.row)"
            >{{ getContent(column, scope.row) }}</el-tag
          >
          <!-- 其他展示列 -->
          <span v-else>
    ...
    const mergedColumns = computed(() => {
      return props.columns.map((originColumn) => {
        ...
        if (column.type === 'operation') {
          ...
          let moreOperation = displayOperations.find(
            (operation) => operation.type === 'more',
          )
          if (moreOperation) {
            moreOperation.operations = moreOperation.operations.filter(
              (op) => !op.hide,
            )
