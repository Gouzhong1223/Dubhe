  ...
  <div class="resource-monitoring-body">
    <div class="progress-wrapper flex flex-around">
      <div class="progress-info">
        ...
        <span
          >CPU: {{ displayData.usedCpu }}核 / {{ displayData.hardCpu }}核</span
        >
      </div>
      <div class="progress-info">
        <el-progress type="dashboard" :percentage="gpuPercentage" />
        <span
          >GPU: {{ displayData.usedGpu }}卡 / {{ displayData.hardGpu }}卡</span
        >
      </div>
      <div class="progress-info">
...
export default {
  ...
  setup(props, { root }) {
    const cpuPercentage = computed(() => {
      ...
      return Math.round(
        (props.resourceInfo.usedCpu / props.resourceInfo.hardCpu) * 100,
      )
    })
    ...
    const gpuPercentage = computed(() => {
      ...
      return Math.round(
        (props.resourceInfo.usedGpu / props.resourceInfo.hardGpu) * 100,
      )
    })
    ...
    const memPercentage = computed(() => {
      ...
      return Math.round(
        (props.resourceInfo.usedMemory / props.resourceInfo.hardMemory) * 100,
      )
    })
