...
export default {
  ...
  computed: {
    params() {
      return {
        up: throttle(1000, this.mouseUp),
        down: throttle(1000, this.mouseDown),
      }
    },
    logTxt() {
    ...
    localLineLimit() {
      return this.lineLimit >= this.logLines * 2
        ? this.lineLimit
        : this.logLines * 2
    },
  },
  mounted() {
    this.$refs.logContent.addEventListener(
      'mousewheel',
      this.watchScroll,
      false,
    )
    this.$once('hook:beforeDestroy', () => {
      this.$refs.logContent.removeEventListener(
        'mousewheel',
        this.watchScroll,
        false,
      )
